...
- 支持一级回复，不实现无限嵌套。
- 状态：待审核、已通过、垃圾、拒绝、用户删除。
- Honeypot、最短填写时间、内容指纹、Bucket4j 限频、每日轮换 IP-HMAC。
- 默认单设备每 10 分钟 3 条、每天 10 条。
- 评论只接受纯文本和少量安全链接，不接受图片和原始 HTML。
- 新评论通过 Outbox 异步发送邮件；邮件失败不影响评论入库。
- 站长可关闭全站或单篇评论。

限频桶放在有界内存缓存中即可；评论的最终审核状态存数据库。单实例重启导致短期限频重置可以接受，但 AI 预算和日额度必须持久化。
...
PWA 离线工具箱放入 V1.1，不让 Service Worker 增加首发缓存和更新复杂度。
...
按单人兼职每周 15–20 小时估算，V1 约 10–12 周。完整 AI 保留，但视觉娱乐功能不得阻塞上线。
...
| 4. 评论与工具 | 第 5 周 | 评论审核/限频、工具管理、5 个内嵌工具，形成可上线非 AI MVP |
| 5. AI 基础 | 第 6 周 | 百炼 Chat/Embedding、SSE、会话、额度、费用台账、取消与超时 |
| 6. RAG 与创作 | 第 7 周 | 分块、混合检索、引用、选区问答、后台创作、缓存、固定问题评测 |
| 7. 创新体验 | 第 8 周 | 知识星图、终端导航、首页标志性动效、性能降级 |
| 8. 2GB 加固 | 第 9 周 | 内存上限、并发闸门、30 分钟压测、故障降级、日志轮转、备份恢复 |
| 9. 部署与合规 | 第 10 周 | 域名/HTTPS、ACR、自动部署、备案材料、AI 标识、隐私/评论规则 |
| 10. 观察期 | 第 11 周 | 小流量灰度 7 天、修复 OOM/5xx/费用问题、正式开放 AI |
| V1.1 | 第 12 周后 | 音乐、频谱、404 小游戏、PWA、体验优化 |

### 6.1 开发优先级

P0（必须）：

- 管理员登录、文章全流程、SSR 阅读、OSS 媒体、搜索、备份。
- 完整 AI 对话、RAG 引用、选区问答、后台创作、额度和硬预算。
- 评论审核与全局 Feature Flag。
- 2GB 内存约束和故障降级。

P1（首发尽量）：

- 知识星图、终端导航、浏览聚合、限时预览链接。

P2（V1.1）：

- 音乐播放器、可视化、小游戏、PWA、复杂动画。

### 6.2 测试计划

后端：

- JUnit 5：文章状态机、Slug、评论审核、额度、费用预留/结算和缓存键。
...
- Playwright：登录、写作发布、评论审核、工具、AI 流和移动端。
...
| 评论垃圾或违法内容 | 先审后发、限频、Honeypot、Feature Flag、审核记录 |
...
| 功能过多延期 | P0 优先；音乐、PWA、小游戏和复杂动画延后 |
...
1. 先完成文章、管理后台、OSS、SSR、搜索、评论和备份，作为非 AI MVP。
2. 接入百炼 Chat/Embedding，完成 SSE、RAG 引用、选区问答和后台创作。
3. 在测试环境用与生产相同的容器上限完成 30 分钟资源压测。
4. 先只向站长开放 AI，观察费用、引用和内存 3–7 天。
5. 再以“每天 5 次、全站并发 3、月预算 50 元”向访客灰度开放。
6. 稳定一周后上线知识星图和终端；音乐、PWA、小游戏留到 V1.1。
